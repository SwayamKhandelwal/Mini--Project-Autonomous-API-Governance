...
async function gatherArchitectureContext() {
  const [serviceRecords, apiRecords, securityRecords, perfRecords] =
    await Promise.all([
      runQuery("MATCH (s:Service) RETURN s"),
      runQuery("MATCH (a:API) RETURN a LIMIT 100"),
      runQuery(
        "MATCH (a:SecurityAlert) WHERE a.resolved = false RETURN a LIMIT 50",
      ),
...
async function generateSuggestions(producer, context) {
  logger.info("Running LLM architecture analysis...", {
...
Return a JSON array of exactly 3 objects. Each object must have: title(max 5 words), category(security/performance/architecture/cleanup), priority(high/medium/low), description(max 20 words), affectedServices(array), estimatedImpact(max 10 words). No markdown. Only the array.`;
  ...
  try {
    ...
    for (const suggestion of suggestions) {
      ...
      logger.info(`Suggestion generated: ${doc.title}`, {
        service: SERVICE_NAME,
      });
    }
  } catch (err) {
  ...
  while (true) {
    try {
      ...
      await consumer.subscribe({
        topics: [
          TOPICS.SECURITY_ALERT,
          TOPICS.PERFORMANCE_ISSUE,
          TOPICS.GOVERNANCE_COMMAND,
        ],
        fromBeginning: false,
      });
      break;
    } catch (err) {
      logger.warn("Kafka not ready, retrying in 10s...", {
        service: SERVICE_NAME,
      });
      await sleep(10000);
    }
  ...
  await consumer.run({
    eachMessage: async ({ topic, message }) => {
      try {
        ...
        const isCritical =
          (topic === TOPICS.SECURITY_ALERT &&
            payload.severity === "critical") ||
          (topic === TOPICS.PERFORMANCE_ISSUE &&
            payload.severity === "critical");
        const isCommand =
          topic === TOPICS.GOVERNANCE_COMMAND &&
          payload.command === "RUN_REFACTOR_ANALYSIS";
