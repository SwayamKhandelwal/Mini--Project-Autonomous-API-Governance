...
async function storeEndpoints(endpoints, serviceName) {
  ...
  logger.info(`Stored ${endpoints.length} endpoints for ${serviceName}`, {
    service: SERVICE_NAME,
  });
}

async function scanService(producer, { serviceName, baseUrl, environment }) {
  logger.info(`Scanning service: ${serviceName}`, { service: SERVICE_NAME });
  const spec = await fetchOpenApiSpec(baseUrl);
  if (!spec) {
    logger.warn(`No OpenAPI spec found for ${serviceName}`, {
      service: SERVICE_NAME,
    });
    return;
  }
...
async function main() {
  ...
  setInterval(
    async () => {
      logger.info("Running scheduled re-scan...", { service: SERVICE_NAME });
      const records = await runQuery("MATCH (s:Service) RETURN s");
      for (const r of records) {
        const s = r.get("s").properties;
        await scanService(producer, {
          serviceName: s.name,
          baseUrl: s.baseUrl,
